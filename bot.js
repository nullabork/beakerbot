...
async function inset(image_spec, insetpath, outpath) {
  ...
  var inset = await Jimp.read(insetpath)
    .then((img) => {
      var w =
        img.bitmap.width > img.bitmap.height ? image_spec.max_width : Jimp.AUTO;
      var h =
        img.bitmap.width <= img.bitmap.height
          ? image_spec.max_height
          : Jimp.AUTO;
...
function getTop(img, image_spec) {
  if (image_spec.valign == "middle")
    return img.bitmap.width > img.bitmap.height
      ? image_spec.top + (image_spec.max_height - img.bitmap.height) / 2
      : image_spec.top;
  else if (image_spec.valign == "bottom")
    return img.bitmap.width > img.bitmap.height
      ? image_spec.top + (image_spec.max_height - img.bitmap.height)
      : image_spec.top;
  else return image_spec.top;
}

function getLeft(img, image_spec) {
  if (image_spec.halign == "middle")
    return img.bitmap.width <= img.bitmap.height
      ? image_spec.left + (image_spec.max_width - img.bitmap.width) / 2
      : image_spec.left;
  else if (image_spec.halign == "right")
    return img.bitmap.width <= img.bitmap.height
      ? image_spec.left + (image_spec.max_width - img.bitmap.width)
      : image_spec.left;
  else return image_spec.left;
}
...
async function doFunny(spec, message, text) {
  ...
  // if they didnt, get the last 5 messages and see if there's any attachments we can inset in the beaker bg
    ...
    var done = false;
    message.channel.messages.fetch({ limit: 30 }).then((msgs) => {
    ...
    if (!done) {
      message.channel.messages.fetch({ limit: 30 }).then((msgs) => {
        msgs.forEach(async (m) => {
          ...
          if (
            !done &&
            (m.cleanContent.startsWith("https://") ||
              m.cleanContent.startsWith("http://"))
          ) {
            done = true;
